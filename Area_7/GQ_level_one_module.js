...
//dragger.style.cssText = 'position:absolute;top:300px;left:300px;width:200px;height:200px;-moz-border-radius:100px;border:1px  solid #ddd;-moz-box-shadow: 0px 0px 8px  #fff;display:none;';
...
function render() {
    ...
    render_counter += 1;
    scene.add(torus_loads.hull_collision);
    // if (render_counter % 120 == 0){


    //     gsap.to(torus_loads.torus_left_wing.material, 0.9, {
    //         //color: color,
    //         opacity: 1,
    //         ease: "back.Out",
    //     });
    //     gsap.to(torus_loads.torus_left_wing.material, 0.1, {
    //         //color: color,
    //         opacity: 0,
    //         ease: "back.Out",
    //     });
    ...
    if (mines_adder_counter < 1
        && render_counter > 10
        && gltf_loads.red_mines_array
    ) {
        mines_adder();

    }
    if (loadingElem.style.display == 'none' && render_counter > 10) {
        protector_returner();
