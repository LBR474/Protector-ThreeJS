...
let CDFL = document.querySelector('#collision_div_finish_level');
...
function render() {
    ...
    // if (collision_div_finish_level.style.display = 'block'){
...
let mines_adder = () => {
    ...
    gltf_loads.red_mines_array.forEach((element, ndx) => {
        if (ndx < gltf_loads.red_mines_array.length * 0.5) {
            scene.add(element);
            element.position.x =
                (ndx + gltf_loads.red_mines_array.length / 4)

                - (gltf_loads.red_mines_array.length * 0.5);
    ...
    gltf_loads.blue_mines_array.forEach((element, ndx) => {
        if (ndx < gltf_loads.blue_mines_array.length * 0.5) {
            scene.add(element);
            element.position.x =
                (ndx + gltf_loads.blue_mines_array.length / 4)

                - (gltf_loads.blue_mines_array.length * 0.5);
    ...
    gltf_loads.green_mines_array.forEach((element, ndx) => {
        if (ndx < gltf_loads.green_mines_array.length * 0.5) {
            scene.add(element);
            element.position.x =
                (ndx + gltf_loads.green_mines_array.length / 4)

                - (gltf_loads.green_mines_array.length * 0.5);
            ...
            // element.position.y = Math.random() * 25;
            element.position.y = -1.5;
            // if (ndx == (gltf_loads.green_mines_array.length - 1)) {
            //     //console.log(gltf_loads.green_mines_array.length );
            //     loadingElem.style.display = 'none';
...
function protector_group_adder() {
    ...
    if (gltf_loads.hull_and_tail) {
        ...
        gltf_loads.protector_group.position.x = 0.5;
...
var level_one_stored_score = sessionStorage.getItem("level_one_stored_score");
//console.log(level_one_stored_score);
...
document.querySelector("#SD_score_par").innerHTML =
    "Score: " + module_score
    // + "<br>Running collision count <br> "
    // + Number(sessionStorage.getItem("level_one_nose_collisions"))
    // + Number(sessionStorage.getItem("level_one_nose_collisions"))
    ;
...
console.log(typeof sessionStorage.getItem("level_one_nose_collisions"));
...
document.querySelector("#c").addEventListener('click', function clicked(e) {
    ...
    if (click_counter == 0) {
        ...
        //console.log(MM1_scores.module_score_level_one);
    ...
    if (sessionStorage.getItem("entered_name")) {
        ...
        if (sessionStorage.getItem("entered_name").length > 0) {
            document.querySelector("#SD_logged_in_par").innerHTML
                = sessionStorage.getItem("entered_name");
