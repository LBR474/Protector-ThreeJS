...
import * as gltf_loads from './gltf_loading_script_3.js';
import * as exp_funcs from './explosion_functions_3.js';
import * as icosa_loads from './icosa_hedron_3.js';
import * as score_update from './score_update_3.js';
import * as gq_l_m_3 from './GQ_level_three_module.js';
...
let module_score_level_three = 0;
...
let mine_mover_2 = () => {
    ...
    if (gq_l_m_3.pause == false
        ...
        && module_score_level_three < 18000
        ...
        if (
            ...
            module_score_level_three += 300;
            score_update.score_update(module_score_level_three, collision_count_nose, collision_count_wing);
...
let new_mine_number_generator = () => {
    ...
    if (module_score_level_three >= 18000) {
        gq_l_m_3.pause == true;
        level_finish(module_score_level_three, 
    ...
    if (gltf_loads.red_mines_array[mine_number]) {
        ...
        gq_l_m_3.pause == true;
        level_finish(module_score_level_three, collision_count_nose, collision_count_wing);
        return;
    } else {
        cancelAnimationFrame(mine_mover_2);
        new_mine_number_generator();
...
let level_finish = (module_score, collision_count_nose, collision_count_wing) => {
    let player;
    ...
    sessionStorage.setItem("level_three_stored_score", module_score_level_three);
    sessionStorage.setItem("level_three_nose_collisions", collision_count_nose);
    sessionStorage.setItem("level_three_wing_collisions", collision_count_wing);
    ...
    document.querySelector("#C_D_F_collision_count").innerHTML =
        document.querySelector("#C_D_F_collision_count").innerHTML =
        "<form><h1>Congratulations" + player + ", Tothian mine field inner zone passed"
        + "<h3>Number of wing collisions (Inner zone): " + collision_count_wing +
        "<br>Number of hull collisions (Inner zone): " + collision_count_nose +
        "<br>Total score (Inner zone): " + module_score +
        ...
        "<br>Running total: " + (module_score + level_one_stored_score_number + level_two_stored_score_number)
        ;

    document.querySelector("#C_D_F_next_level").innerHTML =
        "<a href='../level_four/'><h3>Go to next level?</h3></a>";
